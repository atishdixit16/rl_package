...
def compute_returns(next_value, values, rewards, masks, gamma=0.99):
    R = next_value
    returns = [] 
...
def a2c_algorithm(ENV, NUM_ENV=8,
                  TOTAL_STEPS=4000000, NSTEPS=32, TARGET_UPDATE_FREQUENCY=8000,
                  VF_COEF=1.0, ENT_COEF=0.001, 
                  GAMMA=0.95,
    ...
    assert not TARGET_UPDATE_FREQUENCY % NUM_ENV, 'Invalid target update frequency. For convinience, select such that TARGET_UPDATE_FREQUENCY % NUM_ENV = 0'
    ...
    model_actor = Actor(num_inputs, num_outputs, MLP_LAYERS, MLP_ACTIVATIONS).to(device)
    model_critic = Critic(num_inputs, num_outputs, MLP_LAYERS, MLP_ACTIVATIONS).to(device)
    model_critic_target = Critic_Target(num_inputs, num_outputs, MLP_LAYERS, MLP_ACTIVATIONS).to(device)
    ...
    while steps < total_steps:
        ...
        for _ in range(num_steps):
            ...
            value = model_critic_target(state)
            ...
            if steps % PRINT_FREQ==0:
                test_reward = np.mean([test_env(env, model_actor) for _ in range(N_TEST_ENV)])
            ...
            if steps % TARGET_UPDATE_FREQUENCY ==0:
                for target_param, param in zip(model_critic_target.parameters(), model_critic.parameters()):
                    target_param.data.copy_(param.data)

        next_state = torch.FloatTensor(next_state).to(device)
        next_value = model_critic_target(next_state)
        returns = compute_returns(next_value, values, rewards, masks, GAMMA)
        ...
        actor_loss  = (log_probs * advantage).mean() + ENT_COEF*entropy
        ...
        advantage_batch = advantage[torch.randperm(num_steps*num_envs)[:128]  ]
        critic_loss = VF_COEF* advantage_batch.pow(2).mean()
